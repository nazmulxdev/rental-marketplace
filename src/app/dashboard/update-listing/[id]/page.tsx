...
export default async function UpdateListingPage({ params }: Props) {
  ...
  const ownerMatches =
    (doc.ownerAdminId instanceof ObjectId &&
      doc.ownerAdminId.toString() === userId) ||
    doc.ownerAdminId === userId;
    ...
    <section className="space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-base-content/70 max-w-2xl">
          Modify details, adjust status visibility, and keep information
          accurate.
        </p>
      </div>
      <UpdateListingForm
