...
const steps = [
  {
    ...
    icon: <FaSearchLocation className="text-2xl md:text-4xl text-indigo-500" />,
  },
  {
    id: 2,
    title: "Connect",
    description: "Chat instantly with landlords or agents, ask questions, and schedule visits.",
    icon: <FaComments className="text-2xl md:text-4xl text-indigo-500" />,
  },
  {
    id: 3,
    title: "Book",
    description: "Securely pay deposits or rent online and confirm your booking hassle-free.",
    icon: <FaCalendarCheck className="text-2xl md:text-4xl text-indigo-500" />,
  },
];
        ...
        <div className="relative flex flex-col md:flex-row items-center justify-between gap-12">
          ...
          {steps.map((step, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="flex flex-col items-center text-center bg-base-200 p-6 rounded-2xl shadow-lg w-full md:w-1/3"
            >
              {/* Icon */}
              ...
              {/* Description */}
              <p className="text-card-text text-sm">{step.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
